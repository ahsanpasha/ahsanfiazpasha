  ...
  return (
    <section className="min-h-screen flex items-center relative overflow-hidden px-10 md:px-12 py-24 pt-16 md:pt-28">
      ...
      <div className="absolute inset-0 pointer-events-none" style={{
        ...
        `,
        backgroundSize: "50px 50px",
        maskImage: "radial-gradient(ellipse 50% 45% at 50% 50%, black 20%, transparent 70%)",
        WebkitMaskImage: "radial-gradient(ellipse 50% 45% at 50% 50%, black 20%, transparent 70%)",
      }} />

      {/* Soft gradient blobs */}
      <div className="absolute inset-0 pointer-events-none" style={{
        background: "radial-gradient(ellipse 45% 40% at 15% 45%, hsl(var(--primary) / 0.06) 0%, transparent 100%)",
      }} />
      <div className="absolute inset-0 pointer-events-none" style={{
        background: "radial-gradient(ellipse 35% 30% at 80% 30%, hsl(var(--accent) / 0.04) 0%, transparent 100%)",
      }} />
      <div className="absolute inset-0 pointer-events-none" style={{
        background: "radial-gradient(ellipse 30% 25% at 60% 80%, hsl(var(--primary) / 0.03) 0%, transparent 100%)",
      }} />
      ...
      <div className="w-full max-w-[78rem] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        ...
        <div>
          ...
          <h1 className="font-display font-black leading-[0.9] tracking-tight mb-5">
            <span className={`block text-4xl sm:text-5xl md:text-6xl transition-all duration-700 delay-200 ${loaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
              <span className="text-primary">Ahsan</span>{" "}
              <span className="text-foreground">Fiaz</span>
            </span>
            <span className={`block text-4xl sm:text-5xl md:text-6xl text-foreground mt-1 transition-all duration-700 delay-400 ${loaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
              Pasha
            </span>
          </h1>
          ...
          {/* Actions */}
          <div className={`flex items-center gap-4 flex-wrap transition-all duration-700 delay-700 ${loaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
            <a
              href="#projects"
          ...
          {/* Socials */}
          <div className={`flex items-center gap-3 mt-8 transition-all duration-700 delay-[800ms] ${loaded ? "opacity-100" : "opacity-0"}`}>
            {[
              { href: "https://github.com/ahsanpasha", icon: Github, label: "GitHub" },
              { href: "https://www.linkedin.com/in/ahsan-fiaz-pasha", icon: Linkedin, label: "LinkedIn" },
              { href: "mailto:ahsanfiazpasha@gmail.com", icon: Mail, label: "Email" },
            ].map((s) => (
              <a
                key={s.label}
                href={s.href}
              ...
              <div className="p-5 font-mono text-[12px] leading-[1.8] bg-background">
                <div className="flex">
                  ...
                  <div className="flex-1 space-y-0">
                    <p className="text-muted-foreground/30 italic">{"// quick overview"}</p>
                    <p>&nbsp;</p>
                    <p>
                    ...
                    <p className="text-muted-foreground/25 italic">{"// → shipping pixels..."}</p>
                  </div>
                </div>
              </div>
